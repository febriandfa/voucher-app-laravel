...
import { BreadcrumbItem, MVoucherType, Outlet, Voucher } from '@/types';
...
export default function EditVoucher() {
    const { mVoucherTypes, outlets, voucher } = usePage().props as { mVoucherTypes?: MVoucherType[]; outlets?: Outlet[]; voucher?: Voucher };
    ...
    const { data, setData, patch, errors, processing } = useForm<Required<FormData>>({
        outlet_id: voucher?.outlet_id || 0,
        m_voucher_type_id: voucher?.m_voucher_type_id || 0,
    ...
    const handleOnSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(route('voucher.update', voucher?.id));
    };
